...
/**
 * Helper class used from the SW workflow actions to update the queries being constructed.
 */
@ApplicationScoped
public class QueryAnswerServiceHelper {
    ...
    public void registerAnswer(JsonNode workflowData, KogitoProcessContext context) {
        ...
        try {
            executeOnWorkerThread(() -> updateQueryRecord(processInstanceId, RESOLVED, answer));
        } catch (Exception e) {
    ...
    public void registerError(JsonNode workflowData, KogitoProcessContext context) {
        ...
        try {
            executeOnWorkerThread(() -> updateQueryRecord(processInstanceId, ERROR, error));
        } catch (Exception e) {
            throw new QueryAnswerServiceException("An error was produced during answer registering: " + e.getMessage(), e.getCause());
        }
    }
    ...
    private void executeOnWorkerThread(Runnable action) {
        try {
            managedExecutor.runAsync(action).get(1000, TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new QueryAnswerServiceException(e.getMessage(), e);
